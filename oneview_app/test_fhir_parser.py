    ...
    "entry": [
        {
            "resource": {
                ...
                "address": [
                    ...
                    { # Another address, not home
                        "use": "work",
                        "line": ["456 Business Rd"],
                        "city": "Workville",
                        "state": "CA",
                        "postalCode": "90211",
                    }
                ...
                "communication": [
                    ...
                    {
                        "language": {"coding": [{"system": "urn:ietf:bcp:47", "code": "es", "display": "Spanish"}], "text": "Spanish"},
                        "preferred": False
                    }
                ],
                "telecom": [
        ...
        # --- MedicationRequest and Medication Resources for MOCK_PATIENT_BUNDLE_FULL ---
        { # MedicationRequest using medicationCodeableConcept
            ...
            "resource": {
                ...
                "medicationCodeableConcept": {
                    "coding": [{"system": "http://www.nlm.nih.gov/research/umls/rxnorm", "code": "833676", "display": "Lisinopril 10 MG Oral Tablet"}],
                    "text": "Lisinopril 10 MG Oral Tablet"
                },
                "subject": {"reference": "Patient/patient-1"},
                "authoredOn": "2022-01-01T10:00:00Z",
                "requester": {"display": "Dr. Eva Core"},
                "dosageInstruction": [{"text": "1 tablet by mouth daily"}]
            }
        },
        { # MedicationRequest using medicationReference
            "fullUrl": "urn:uuid:medreq-2",
            "resource": {
                "resourceType": "MedicationRequest",
                "id": "medreq-2",
                "status": "completed",
                "intent": "order",
                "medicationReference": {"reference": "urn:uuid:med-metformin"}, # Points to Medication below
                "subject": {"reference": "Patient/patient-1"},
                "authoredOn": "2021-11-15T09:30:00Z",
                "requester": {"display": "Dr. Eva Core"},
                # No dosageInstruction for this one
            }
        },
        { # Corresponding Medication resource for medreq-2
            ...
            "resource": {
                ...
                "code": {
                    ...
                    "text": "Metformin 500mg tab" # Text slightly different from display
                }
            }
        },
        { # MedicationRequest with a reference to a Medication not in this bundle
            "fullUrl": "urn:uuid:medreq-3",
            "resource": {
                "resourceType": "MedicationRequest",
                "id": "medreq-3",
                "status": "active",
                "intent": "order",
                "medicationReference": {"reference": "Medication/med-aspirin-external", "display": "Aspirin 81mg (External)"},
                "subject": {"reference": "Patient/patient-1"},
                "authoredOn": "2023-01-20T10:00:00Z",
                "requester": {"display": "Dr. Self Prescribe"},
                "dosageInstruction": [{"text": "1 tablet daily as needed"}]
            }
        }
    ]
}
...
class TestFhirParser(unittest.TestCase):
    ...
    def test_parse_patient_full(self):
        ...
        self.assertEqual(parsed["address_full"], "123 Main St, Apt 4B, Anytown, CA, 90210, USA")
        self.assertEqual(parsed["marital_status"], "Married")
        self.assertEqual(parsed["preferred_language"], "English")
        ...
        # Test medications
        self.assertIsNotNone(parsed.get("medications"))
        self.assertEqual(len(parsed["medications"]), 3)

        med1 = next(m for m in parsed["medications"] if m["name"] == "Lisinopril 10 MG Oral Tablet")
        self.assertEqual(med1["authored_on"], "2022-01-01T10:00:00Z")
        self.assertEqual(med1["prescriber"], "Dr. Eva Core")
        self.assertEqual(med1["dosage"], "1 tablet by mouth daily")
        self.assertEqual(med1["status"], "active")

        med2 = next(m for m in parsed["medications"] if m["name"] == "Metformin 500mg tab") # Name from Medication.code.text
        self.assertEqual(med2["authored_on"], "2021-11-15T09:30:00Z")
        self.assertEqual(med2["prescriber"], "Dr. Eva Core")
        self.assertIsNone(med2["dosage"]) # No dosageInstruction
        self.assertEqual(med2["status"], "completed")
        
        med3 = next(m for m in parsed["medications"] if m["name"] == "Aspirin 81mg (External)") # Name from MedicationReference.display
        self.assertEqual(med3["authored_on"], "2023-01-20T10:00:00Z")
        self.assertEqual(med3["prescriber"], "Dr. Self Prescribe")
        self.assertEqual(med3["dosage"], "1 tablet daily as needed")
        self.assertEqual(med3["status"], "active")
    ...
    def test_parse_patient_minimal(self):
        ...
        self.assertIsNone(parsed["address_full"])
        self.assertIsNone(parsed["marital_status"])
        self.assertIsNone(parsed["preferred_language"])
        self.assertEqual(len(parsed["recent_encounters"]), 0)
        self.assertEqual(len(parsed["diagnoses"]), 0)
        self.assertEqual(len(parsed.get("medications", [])), 0) # Ensure empty medications list
